...
// KEY FIXES:
// 1. Each page processed separately (prevents cross-page Y mixing)
// 2. Section (pays/allowances/deductions) persists across pages
// 3. Any A-code in Deductions = recovery, subtract from pays/allowances
// 4. All A-codes found via segment scan (handles merged G+A lines)
...
export async function parsePayrollPDF(file) {
  ...
  // Build lines per page (keeps Y ordering within each page correct)
  // Then concatenate in page order 1 → 2 → 3
  const allLines = [];
  ...
  console.log('=== PDF Lines (first 70) ===');
  allLines.slice(0, 70).forEach((l, i) => console.log(i, l));
...
// Group items from ONE PAGE into lines, sorted top-to-bottom
function buildLines(items) {
  ...
  return Object.keys(groups)
    .map(Number)
    .sort((a, b) => b - a)           // descending Y = top to bottom (pdf.js coords)
    .map(y =>
...
function parse(lines) {
  ...
  // section persists across pages - do NOT reset between pages
  let section = null;

  for (const line of lines) {
    if (!line.trim()) continue;

    // Section detection - carries over page boundaries
    ...
    // Find ALL A-codes in line (handles merged G-code + A-code rows)
    CODE_RE.lastIndex = 0;
    const codeMatches = [];
    let cm;
    ...
    for (let i = 0; i < codeMatches.length; i++) {
      ...
      const amts = getAmounts(line.slice(idx, segEnd));
      if (amts.length === 0) continue;

      // Last amount = Total Employees column (7th column in PDF)
      ...
      if (section === 'pays' || section === 'allowances') {
        ...
        // Any A-code in Deductions = salary recovery → subtract from budget total
        deductionsA[code] = (deductionsA[code] || 0) + target;
        console.log(`[deduction] ${code}: ${target.toLocaleString()}`);
      }
    }
  }

  // Subtract recoveries from pays/allowances
  ...
  for (const [code, deductAmt] of Object.entries(deductionsA)) {
    if (code in amounts) {
      ...
      console.log(`Recovery applied: ${code} ${before.toLocaleString()} - ${deductAmt.toLocaleString()} = ${amounts[code].toLocaleString()}`);
    }
  }

  // Remove zero or negative results
