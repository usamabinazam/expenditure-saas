...
export function calculateStatement(thisMonthData, previousStatement, heads) {
  const result = {
    pays: [],
    allowances: [],
    pays_subtotal: { this_month: 0, previous: 0, total: 0, saving: 0, excess: 0 },
    allowances_subtotal: { this_month: 0, previous: 0, total: 0, saving: 0, excess: 0 },
    grand_total: { this_month: 0, previous: 0, total: 0, saving: 0, excess: 0 },
  };
  ...
  if (previousStatement && previousStatement.data) {
    ...
    [...(prevData.pays || []), ...(prevData.allowances || [])].forEach((item) => {
      prevLookup[item.code] = item.total;
    });
  }

  // Process both sections
  ['pays', 'allowances'].forEach((section) => {
  ...
  // Grand total
  ['this_month', 'previous', 'total', 'saving', 'excess'].forEach((key) => {
    result.grand_total[key] =
      result.pays_subtotal[key] + result.allowances_subtotal[key];
  });
